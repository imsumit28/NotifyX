// Screen components for NotifyX

const { Icon } = window;
const { Sparkline, AreaChart, RingChart, BarChart, Heatmap } = window.Charts;
const { STATS, JOBS_SERIES, ACTIVITY, QUEUE_JOBS, NOTIFICATIONS, DEFAULT_PREFS, PREF_DEFS } = window.NTFX_DATA;

// ---------- Dashboard ----------
// "Live" updating clock — establishes the product feels real
const useLastUpdated = () => {
  ...
  const refresh = () => {
    setRefreshing(true);
    setTimeout(() => { setSecs(0); setRefreshing(false); }, 700);
  };
    ...
    <div className="page">
      ...
      <section className="hero-card">
        ...
        <div className="hero-stats">
          <InlineStat label="Total dispatched" value="2,847,193" delta="+4.2%" trend="up" hint="vs prior 24h"/>
          <InlineStat label="In flight"        value="1,284"     delta="+86"   trend="up" hint="active workers"/>
          <InlineStat label="Failed"           value="47"        delta="−12"   trend="down-good" hint="auto-retry queued"/>
          <InlineStat label="Throughput p95"   value="4,127/s"   delta="124ms" trend="neutral" hint="enqueue → deliver"/>
        </div>
      </section>

      {/* Section break — generous whitespace before the secondary data */}
...
const Queue = () => {
  ...
  return (
    <div className="page">
              ...
              {filtered.map(j => (
                <tr key={j.id}>
                  ...
                  <td>
                    ...
                    <div className="mono fg-faint" style={{fontSize:11, marginTop:2}}>{j.error ? <span style={{color:'var(--red)'}}>↳ {j.error}</span> : j.payload}</div>
                  </td>
              ...
              {filtered.length===0 && (
                <tr><td colSpan="6">
                  <div className="empty-state">
                    ...
                    <div className="empty-sub">No jobs match “{filter==='all'?'this filter':filter}” in the selected time window. Try widening the range or clearing filters.</div>
      ...
      {toast && (
        <div className="toast"><Icon name="check" size={14} className="ok"/>{toast}</div>
      )}
    </div>
  );
};
...
const Notifications = () => {
  const [groups, setGroups] = React.useState(NOTIFICATIONS);
  const [tab, setTab] = React.useState('all');

  const totalUnread = groups.reduce((n,g)=>n+g.items.filter(i=>!i.read).length, 0);

  const toggle = (id) => {
    setGroups(prev => prev.map(g => ({...g, items: g.items.map(i => i.id===id ? {...i, read: !i.read} : i)})));
  };
  const markAll = () => setGroups(prev => prev.map(g => ({...g, items: g.items.map(i => ({...i, read: true}))})));

  const filterFn = (i) => tab==='all' ? true : tab==='unread' ? !i.read : i.kind === tab;
  const visible = groups.map(g => ({...g, items: g.items.filter(filterFn)})).filter(g => g.items.length);
      ...
      <div className="card">
        ...
        {visible.length === 0 && (
          <div className="empty-state">
            ...
            <div className="empty-sub">No new {tab==='all'?'notifications':tab+'s'}. We’ll let you know when something needs attention.</div>
          </div>
        )}
        ...
        {visible.map(g => (
          <div className="notif-group" key={g.group}>
            ...
            {g.items.map(n => {
              ...
              return (
                <div key={n.id} className={`notif ${n.read?'read':'unread'}`} onClick={()=>toggle(n.id)}>
                  ...
                  <div className="notif-body">
                    <div className="notif-title">
                      {n.title}
                      {n.batched > 1 && <span className="notif-batch">×{n.batched}</span>}
                    </div>
...
const Settings = () => {
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <div className="eyebrow">elena.park@notifyx.dev</div>
...
// ---------- Metrics ----------
const Metrics = () => {
  const failureRate = 1.34;
  const successRate = 100 - failureRate;
  const channelBars = [4127, 2890, 1240, 3640, 980, 412, 1820];
  const channelLabels = ['00','04','08','12','16','20','24'];
      ...
      <div className="metrics-grid" style={{marginBottom:16}}>
        ...
        <div className="card">
          ...
          <div className="card-body" style={{display:'flex', alignItems:'center', justifyContent:'space-between', gap:24}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:11, color:'var(--fg-faint)', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:8}}>Top error reasons</div>
              {[
      ...
      <div className="metrics-grid" style={{marginBottom:16}}>
        <div className="card">
          <div className="card-h">
            <div>
              ...
              <div className="sub">p95 enqueue → delivery (ms)</div>
            </div>
            <span className="mono" style={{color:'var(--fg-faint)', fontSize:11}}>p50 56ms · p95 124ms · p99 412ms</span>
          </div>
          <div className="chart-wrap">
            <BarChart data={latencyBars} labels={latencyLabels} color="oklch(0.68 0.13 245)" suffix="ms"/>
          </div>
        </div>
        ...
        <div className="card">
          <div className="card-h">
            <div>
              <h3>Throughput</h3>
              <div className="sub">Events per minute · last 24h</div>
            </div>
            <span className="mono" style={{color:'var(--fg-faint)', fontSize:11}}>peak 4,320/min at 12:00 UTC</span>
          </div>
          <div className="chart-wrap">
            <BarChart data={channelBars} labels={channelLabels} color="oklch(0.72 0.13 295)" suffix=""/>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-h">
          <div>
            ...
            <div className="sub">By weekday × hour · the last 7 days</div>
          </div>
